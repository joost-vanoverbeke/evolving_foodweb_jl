...
struct Ecol_parameters
    ...
    function Ecol_parameters(;
        ...
        return new(grid, torus, env_range, env_step, patches, m, rho, m_tl, m_power, in_rate, out_rate, d, d_power, species, trophic_levels, bodymass_tl, tl_species, uptake_pars, i_power, uptake_tl, resource_conversion, resource_assimilation, assimilation_eff, conversion_tl, scale_uptake, scale_assim)
    end
end
...
mutable struct Individual
    ...
    function Individual(ecol::Ecol_parameters, evol::Evol_parameters, e::Float64, s::Int, gtp::Vector{Int8})
        ...
        mortality_rate = 1 - fitness * (1 -  ecol.d * bm^ecol.d_power)
        return new(species_ID, genotype, phenotype, fitness, mortality_rate)
    end
...
function evolving_foodweb_dm(; time_steps = 1)
    init = Init_values();
    ecol = Ecol_parameters(; grid = (X = 5, Y = 2), in_rate = 200., scale_uptake = 1.);
...
maximum(dm.c_b_resource)
maximum(dm.c_d_resource)

maximum(dm.c_b_ind[ecol.tl_species .== 1, :])
maximum(dm.c_b_ind[ecol.tl_species .== 2, :])
maximum(dm.c_b_ind[ecol.tl_species .== 3, :])

maximum(dm.c_d_ind[ecol.tl_species .== 1, :])
maximum(dm.c_d_ind[ecol.tl_species .== 2, :])
maximum(dm.c_d_ind[ecol.tl_species .== 3, :])
