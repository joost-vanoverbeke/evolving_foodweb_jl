...
for (f in files_sex) {
  ...
  data_div_alpha <-
    ...
    filter(N > 0) %>% 
    group_by(CC, time_CC, pre_CC, post_CC, e_step_local, rep_type, m, mu, run, time, patch, X, Y, trophic_level) %>%
    ...
    group_by(CC, time_CC, pre_CC, post_CC, e_step_local, rep_type, m, mu, run, time, trophic_level) %>%
  ...
  data_div_gamma <-
    ...
    # mutate(run = ordered(run)) %>% 
    filter(N > 0) %>% 
    group_by(CC, time_CC, pre_CC, post_CC, e_step_local, rep_type, m, mu, run, time, species, trophic_level) %>%
  ...
  data_div_join <-
    ...
    select(-run) %>% 
    group_by(CC, time_CC, pre_CC, post_CC, e_step_local, rep_type, m, mu, time, trophic_level) %>%
    # group_by(mu, time) %>% 
  ...
  data_shift_f <-
    ...
    ungroup() %>% 
    group_by(CC, time_CC, pre_CC, post_CC, e_step_local, rep_type, m, mu, run, species, trophic_level) %>%
