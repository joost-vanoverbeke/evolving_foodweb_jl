...
data_div_alpha <-
  ...
  filter(N > 0) %>% 
  group_by(scenario, mu, run, time, patch, X, Y, trophic_level) %>%
  ...
  group_by(scenario, mu, run, time, trophic_level) %>%
...
data_div_gamma <-
  ...
  mutate(run = ordered(run)) %>% 
  filter(N > 0) %>% 
  group_by(scenario, mu, run, time, species, trophic_level) %>%
...
data_div <-
  ...
  select(-run) %>% 
  group_by(scenario, mu, time, trophic_level) %>%
  # group_by(mu, time) %>% 
...
data_shift <-
  ...
  ungroup() %>% 
  group_by(scenario, mu, run, species, trophic_level) %>%
...
data_div %>% 
  ...
  # scale_y_log10() +
  facet_grid(level ~ trophic_level, labeller = "label_both", scales = "free_y") +
  scale_x_discrete(labels = NULL)
